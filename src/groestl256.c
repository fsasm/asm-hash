...
void groestl256_init (groestl256_context* ctxt) {
	...
	block_init_with_end (&ctxt->b, BLOCK_SIZE_512, ctxt->buffer, process_blocks, end_hash, ctxt->hash);
...
void groestl256_finalize (groestl256_context* ctxt) {
	block_util_finalize (&ctxt->b, BLOCK_LENGTH_64 | BLOCK_BIG_ENDIAN | BLOCK_SIMPLE_PADDING | BLOCK_COUNT);
}
...
void add_round_constants_p (uint8_t state[8][8], unsigned int round) {
	for (uint_fast8_t i = 0; i < 8; i++) {
		state[0][i] ^= round ^ (i << 4);
	}
}

void add_round_constants_q (uint8_t state[8][8], unsigned int round) {
	for (uint_fast8_t i = 0; i < 8; i++) {
...
void shift_bytes_p (uint8_t state[8][8]) {
	uint8_t temp[8];
...
void shift_bytes_q (uint8_t state[8][8]) {
	uint8_t temp[8];
...
void sub_bytes (uint8_t state[8][8]) {
	for (uint_fast8_t row = 0; row < 8; row++) {
		for (uint_fast8_t column = 0; column < 8; column++) {
...
static void print_matrix (char* name, uint8_t block[8][8]) {
	printf ("%s:\n", name);
	for (uint_fast8_t x = 0; x < 8; x++) {
		for (uint_fast8_t y = 0; y < 8; y++) {
			printf ("%2.2X ", block[x][y]);
		}
		printf ("\n");
	}
}
...
void groestl256_process_block (const uint8_t block[64], uint8_t hash[8][8], unsigned int n) {
	...
	print_matrix ("initial state", state);
	...
	for (uint_fast8_t r = 0; r < 10; r++) {
		...
		print_matrix ("state - add round constant", state);
		sub_bytes (state);
		print_matrix ("state - sub bytes", state);
		shift_bytes_p (state);
		print_matrix ("state - shift bytes", state);
		mix_bytes (state);
		print_matrix ("state - mix bytes", state);
	}
	...
	for (uint_fast8_t r = 0; r < 10; r++) {
		...
		print_matrix ("block - add round constant", message);
		sub_bytes (message);
		print_matrix ("block - sub bytes", message);
		shift_bytes_q (message);
		print_matrix ("block - shift bytes", message);
		mix_bytes (message);
		print_matrix ("block - mix bytes", message);
	}
...
void end_hash (block* b) {
	...
	for (uint_fast8_t row = 0; row < 8; row++) {
		for (uint_fast8_t column = 0; column < 8; column++) {
			hash[row * 8 + column] ^= state[row][column];
		}
	}
	
	print_matrix ("hash", (uint8_t(*)[8])hash);
}
