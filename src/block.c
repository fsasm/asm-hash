...
void block_util_finalize (block* b, block_flag flags) {
	...
	if ((flags & BLOCK_EXTENDED_PADDING) != 0 && block_size == length_index) {
		...
		if ((flags & BLOCK_LITTLE_ENDIAN) != 0) {
			u64_to_u8_le (b->full_size * 8, &buffer[block_size]);
			block_size += 8;
			...
			u64_to_u8_be (b->full_size * 8, &buffer[block_size]);
		}
