...
static void add_round_constants_q(uint8_t state[8][16], unsigned int round) {
	for (uint_fast8_t i = 0; i < 16; i++) {
...
static void shift_bytes_p(uint8_t state[8][16]) {
	uint8_t temp[16];
...
static void shift_bytes_q(uint8_t state[8][16]) {
	uint8_t temp[16];
...
static void sub_bytes(uint8_t state[8][16]) {
	for (uint_fast8_t row = 0; row < 8; row++) {
		for (uint_fast8_t column = 0; column < 16; column++) {
